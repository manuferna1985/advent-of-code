...
public class Day16 extends Day {
  ...
  protected String part2(String fileContents) throws Exception {
    Character[][] charsMatrix = MatrixUtils.readMatrixFromFile(fileContents, Character.class, s -> s.charAt(0));
    List<Beam> initialBeams = new ArrayList<>();

    for (int x = 0; x < charsMatrix.length; x++) {
      for (int y = 0; y < charsMatrix[x].length; y++) {
        if (x==0) {
          initialBeams.add(new Beam(Point.of(x, y), DOWN));
        } else if (y==0) {
          initialBeams.add(new Beam(Point.of(x, y), RIGHT));
        } else if (x==charsMatrix.length - 1) {
          initialBeams.add(new Beam(Point.of(x, y), UP));
        } else if (y==charsMatrix[x].length - 1) {
          initialBeams.add(new Beam(Point.of(x, y), LEFT));
        }
      }
    }

    return String.valueOf(initialBeams
        .parallelStream()
        .map(b -> getEnergyFrom(charsMatrix, b))
